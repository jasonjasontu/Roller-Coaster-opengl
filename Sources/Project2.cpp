...
std:printf(preamble.c_str());
	...
	// -------------------------
	Shader lightingShader_basic("../Project2/Shaders/lightingShader_basic.vert", "../Project2/Shaders/lightingShader_basic.frag");
	Shader reflectionShader("../Project2/Shaders/reflectionShader.vert", "../Project2/Shaders/reflectionShader.frag");
	Shader skyboxShader("../Project2/Shaders/skyboxShader.vert", "../Project2/Shaders/skyboxShader.frag");
	Shader lightingShader_specular("../Project2/Shaders/lightingShader_specular.vert", "../Project2/Shaders/lightingShader_specular.frag");
	...
	{
		"../Project2/Media/skybox/right.jpg",
		"../Project2/Media/skybox/left.jpg",
		"../Project2/Media/skybox/top.jpg",
		"../Project2/Media/skybox_self/bottom.jpg",
		"../Project2/Media/skybox/back.jpg",
		"../Project2/Media/skybox/front.jpg"
	};

	unsigned int cubemapTexture = loadCubemap(faces);

	// init heatmap
	Heightmap heightmap("../Project2/Media/heightmaps/hflab4.jpg");
	unsigned int heightmap_texture = loadTexture("../Project2/Media/skybox_self/bottom.jpg");
	...
	// load models
	// -----------
	Model ourModel("../Project2/Media/nanosuit/nanosuit.obj");
	...
	//Initial velocity of the camera(roller coaster)
	float time_left = 0.0f;
	float time_step = 0.0f;
	float travel = 0.0f;
	float velocity;
	int camera_index = 0;
	camera.Position = track.cameraTrack[camera_index].pos;
	camera.Track_front = glm::normalize(track.cameraTrack[camera_index + 1].pos - track.cameraTrack[camera_index].pos);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (deltaTime < 0.1f) {
			...
			if(freeMode){
			//code to get a track from flying camera
				t_track += deltaTime;
				if (t_track > 1.0f) {
			...
			time_left += deltaTime*animation_speed;
			while (time_left > time_unit) {
				velocity = sqrt(2.0*g*(Init_h - camera.Position.y));
				travel += velocity*time_unit;

				while (travel > track.cameraTrack[camera_index].dis) {
					travel -= track.cameraTrack[camera_index].dis;
					camera_index = (camera_index + 1) % track.cameraTrack.size();

					if (!freeMode)
						camera.ProcessTrackMovement(track.cameraTrack[camera_index].pos, track.cameraTrack[camera_index].up);
				}
		...
		if (!freeMode) {
			...
			reflectionShader.use();
			track.Draw(reflectionShader, diffuseMap);
		}
		...
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f)); // Rotated it towards the light
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));	// it's a bit too big for our scene, so scale it down
		
		lightingShader_specular.setMat4("model", model);
		ourModel.Draw(lightingShader_specular);
...
void processInput(GLFWwindow *window)
{
	...
	if (somethingPressed && last_pressed < currentFrame - 0.5f || last_pressed == 0.0f)
	{
		...
		if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS){
			freeMode ? freeMode = false : freeMode = true;
			camera.onTrack = !freeMode;
		}
